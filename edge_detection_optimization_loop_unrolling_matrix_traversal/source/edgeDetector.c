...
#define TYPE_FILTER GAUSSIAN //GAUSSIAN or MEDIAN
...
int main(int nargs, char **argv)
{
    ...
    for (int file_i = 1; file_i < nargs; file_i++)
    {
        ...
        int r, g, b, i, j;
        //change here (loop order j, i)
        //parallel comment: no parallelization because it made the program slower, makes sense because its a really simple operation and creating threads is more costly
        for (j = 0; j < height; j++)
        {
            for (i = 0; i < width; i++)
            {
...
#define PIXEL_EDGES(x, y) (edges[(x) + (y)*width_edges]);
        ...
        {
            //change here loop order (i, j)
            //parallelize comment: here, if the image is small enough, there will be false sharing as all write to the same zone, but we
            //can't solve it as it is not an accumulation that we can reduce nor can we use a private variable as then 
            //we would have to copy it to the common zone and will cause false sharing anyways.

            printf("[info] Using median denoising...\n");
            #pragma omp parallel for private(i, p1, p2, x, y, k, array)
            for (j = radius; j < height_edges - radius; j++)
            {
                array = malloc((2 * radius + 1) * (2 * radius + 1));
                for (i = radius; i < width_edges - radius; i++)
                {
                    ...
                    //This part is too simple to parallelize it, a single thread can do it.
        ...
        // More classic gaussian filter
            ...
            double sum = 0;
            //change here (loop order i,j) THE IMPORTANT ONE
            #pragma omp parallel for private(i, p1, p2, x, y, sum)
            for (int j = radius; j < height_edges - radius; j++)
            {
                for (int i = radius; i < width_edges - radius; i++)
                {
                    ...
                    //change here (loop order p1, p2)
                    //We could use reduction here (+ : sum) but there are so few iterations we think it will not improve performance
