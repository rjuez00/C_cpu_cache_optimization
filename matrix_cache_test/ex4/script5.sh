...
fDAT_og=ex4/results5/
fPNG_cache=ex4/results5/plot_cache.png

# Remove the DAT and the PNG files.
rm -Rf ex4/results5
mkdir ex4/results5
mkdir ex4/results5/plots
...
echo "cache:"
# Basically the same as iterations but with cache_associativity in LL too.
for ((cache_associativity = cache_associativity_inicio ; cache_associativity <= cache_associativity_final ; cache_associativity = cache_associativity * 4)); do
	...
	for ((N = Ninicio ; N <= Nfinal ; N += Npaso)); do
		...
		valgrind --LL=8388608,1,64 --I1=8192,$cache_associativity,64 --D1=8192,$cache_associativity,64 --tool=cachegrind --cachegrind-out-file=cache.dat ./slow $N &> /dev/null
...
fPNG_og=ex4/results5/plots/plot
...
files=system('ls -1B ex4/results5/*.dat')
